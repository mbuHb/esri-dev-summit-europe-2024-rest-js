...
form.addEventListener("submit", function (event) {
  ...
  alert("Geocoded address: " + JSON.stringify(results, null, 2));
});
...
streetAddressInput.addEventListener(
  ...
  function (event) {
    ...
    suggest(comboBoxText, {
      ...
      sourceCountry: "DEU", // the source of who is providing the suggestions
    }).then((response) => {
      const htmlStr = response.suggestions
        .map(({ magicKey, text }) => {
          return `<calcite-combobox-item
            value="${magicKey}:::${text}" text-label="${text}"></calcite-combobox-item>`;
        })
        .join();

      streetAddressInput.innerHTML = htmlStr;
      streetAddressInput.open = true;
    });
